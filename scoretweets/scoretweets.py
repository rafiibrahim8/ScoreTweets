...
def run_score_tweet(env_path, config_path, required_words, is_delete):
    ...
    fpc = FacebookPostClient(config, env['facebook_group_id'], debug_discord, is_delete)
    dpc = DiscordPostClient(env['discord_hook'], debug_discord)
    stc = ScoreTweetClient(env, dpc, fpc, required_words, debug_discord)
    dlt = DeleteClient(config.get('keep_day', 2), fpc, debug_discord)
    ...
    loop.create_task(dlt.run())
    loop.run_forever()

def main():
    env_path = os.path.join(os.path.expanduser(CONFIG_DIR), 'env.json')
    config_path = os.path.join(os.path.expanduser(CONFIG_DIR), 'config.json')
    ...
    parser.add_argument('-d', '--delete', action='store_true', dest='is_delete', default=False, help='Delete facebook post left by this program.')
    ...
    if args.is_configure:
        ...
        run_score_tweet(args.env_path, config_path, args.required_word, args.is_delete)
